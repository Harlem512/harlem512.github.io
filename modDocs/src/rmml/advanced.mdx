...
RMML implements multiple safety features to prevent mods from crashing your game on start up, preventing you from accessing RMMM's UI. his is a list of changes Dev mode currently supports. You can implement your own Dev features in your own mods.
...
The easiest way to disable dev mode is with a game start script:
...
1. _Don't cause permanent damage_. Your mod should be able to be uninstalled without preventing Rusted Moss or another mod from working (unless it's a dependency). The only exception is save files: save files changed while your mod is installed are free game, but your impact should be minimized.
