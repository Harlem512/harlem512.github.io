...
All multi-line code blocks should use two spaces for indentation. GML code blocks are allowed to use the [Allman](https://en.wikipedia.org/wiki/Indentation_style#Allman_style) brace style, since it's what UTMT exports, but all other code should use the [One True Brace](https://en.wikipedia.org/wiki/Indentation_style#One_True_Brace) style. Variable names should use `snake_case`, if possible.
...
Most of the larger mod examples use Rusted Moss Mod Loader's [syntax](/rm-docs/rmml/advanced#md), since it supports multiple-line Catspeak code, which is easier for humans to read.
